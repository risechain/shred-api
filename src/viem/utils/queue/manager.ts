...
export class RequestQueueManager implements RequestQueue {
  ...
  async add(
    request: Omit<QueuedRequest, 'id' | 'createdAt' | 'retryCount' | 'resolve' | 'reject'>
  ): Promise<any> {
    ...
    try {
      ...
      if (!(await this.isConnectionReady())) {
        ...
        if (request.retryCount < this.maxRetries) {
          ...
          const delay = this.connectionManager?.getStatus() === 'connecting' ? 500 : this.retryDelay
          setTimeout(() => {
  ...
  getStats(): RequestQueueStats {
    return {
      ...
      failed: this.failed,
      avgProcessingTime: this.processed > 0 
...
export function getRequestQueue(transport: any, config?: RequestQueueConfig): RequestQueueManager {
